...
private function attachDebuggers(PendingRequest $client): PendingRequest
{
    ...
    {
        if ($nextUrl) {
            $response = $this->http()->acceptJson()->get($nextUrl);
        } else {
            $query = [
                '$top'   => $top,
                'expand' => ['Lines', 'Customer'], // lean list
            ];
            if ($filter) $query['$filter'] = $filter;

            // NOTE: list endpoint is lowercase per what worked in tinker
            $response = $this->http()->acceptJson()->get('salesorders', $query);
        }
    ...
    public function fetchSingleOrder(string $type, string $orderId): array
    {
        ...
        $resp = $this->http()->acceptJson()->get($url, ['expand' => ['Lines','Customer']]);
    ...
    public function pushVismaOrder(VismaOrder $order): VismaOrder
    {
        ...
        $get    = $this->http()->acceptJson()->get($getUrl, ['expand' => ['Lines','Customer']]);
        ...
        if (!empty($headerPatch)) {
            $patch = $this->httpWrite()->acceptJson()
        ...
        $remote = $this->http()->acceptJson()->get($getUrl, ['expand' => ['Lines']])->json();
        ...
        if (!empty($updateLines)) {
            $linesResp = $this->httpWrite()->acceptJson()
                ->withHeaders(['If-Match' => $etag])
                ->patch($getUrl . '/lines', ['update' => $updateLines]);

            if (in_array($linesResp->status(), [409, 412])) {
                $fresh = $this->fetchSingleOrder($order->type, $order->order_id);
                $this->persistOrderFromPayload($fresh);
    ...
    public function persistOrderFromPayload(array $payload): VismaOrder
    {
        ...
        $order->fill([
            ...
            'currency'                => $this->stringValue(data_get($payload, 'currency')),
    ...
    private function http(): PendingRequest
    {
        return Http::withHeaders([
                // swap to live tenant id when you go live
                'ipp-tenant-id' => '87998127-00cc-11ec-b60b-0638767d04b5',
            ])
            ->withToken($this->getAccessToken(false)) // read token
            ->baseUrl(rtrim(config('services.visma.sales_order_base_url') ?? self::DEFAULT_BASE_URL, '/') . '/')
            ->timeout(60);

                return $this->attachDebuggers($client); // <-- add this line
    ...
    private function httpWrite(): PendingRequest
    {
        return Http::withHeaders([
            ...
            ->withToken($this->getAccessToken(true))  // write token
            ->baseUrl(rtrim(config('services.visma.sales_order_base_url') ?? self::DEFAULT_BASE_URL, '/') . '/')
    ...
    private function getAccessToken(bool $write = false): string
    {
        ...
        // DEV vs LIVE tenant — toggle when switching environments
        $tenantId = '87998127-00cc-11ec-b60b-0638767d04b5'; // dev
        // $tenantId = '3378aa46-fb61-11eb-b60b-0638767d04b5'; // live
    ...
    // --- NEW: Create (POST) a Sales Order in Visma ------------------------------
public function createVismaOrder(VismaOrder $order): VismaOrder
{
    // Ensure we have items
    $order->loadMissing('items');

    // Build payload from local model
    $payload = $this->buildCreatePayloadFromModel($order);

    // POST /SalesOrders (PascalCase, as used elsewhere in your service)
    $resp = $this->httpWrite()->acceptJson()->post('SalesOrders', $payload);
    ...
    // Visma typically returns the created order (often 201/202)
    $created = $resp->json();
    if (!is_array($created)) {
        throw new \RuntimeException('Unexpected response from Visma when creating order.');
    }

    // Persist & return (keeps version/lines in sync with Visma)
    return $this->persistOrderFromPayload($created);
}

/**
 * Build a minimal, valid payload for POST /SalesOrders from a local VismaOrder.
 * Sends only non-null fields; pads numeric order ids to 6 chars.
 */

// In App\Services\VismaOrderService
...
public function createOrderInVisma(VismaOrder $order): VismaOrder
{
    ...
    // ---- Build Visma payload (use IDs that exist in your tenant) ----
    // If your Filament field `customer_id` stores the Visma customer number (e.g. "10000"), this is correct.
    $customerId  = (string) $order->customer_id;

    // Location is required if the customer’s default is not set / not resolvable.
    // Use the value the user chose (order.location) or fall back to a known good one in your tenant ("Main").
    $locationId  = $order->location ?: 'Main';

    // Terms are required if not defaulted on customer in Visma. Put a safe default you actually have in Visma.
    // Recommend moving this to config/services.php: 'visma' => ['default_terms_id' => 'NET30']
    $termsId     = config('services.visma.default_terms_id', 'NET30');

    // Optional shipping date normalization (only send if valid)
    $scheduledDate = $this->normalizeIsoDate($order->shipping_scheduled_date);

    $payload = array_filter([
        'type'         => $order->type ?: 'BB',
        'currencyId'   => $order->currency ?: 'NOK',
        'description'  => $order->description,
        'customer'     => [
            'id'         => $customerId,          // <- must exist in Visma
            'locationId' => $locationId,          // <- e.g. "Main"
        ],
        'paymentSettings' => [
            'termsId' => $termsId,                // <- must exist in Visma
        ],
        // Only include shipping if you have a valid date
        'shipping' => $scheduledDate ? ['scheduledDate' => $scheduledDate] : null,
        'orderLines'   => $lines->map(function ($i) {
            // IMPORTANT: for create you must send inventoryId here (flat), not nested inventory.id
            return array_filter([
                'inventoryId'     => (string) $i->inventory_id, // <- must exist in Visma
                'quantity'        => is_null($i->quantity) ? null : (float) $i->quantity,
...
private function buildCreatePayloadFromModel(VismaOrder $order): array
{
    ...
    $payload = array_filter([
        ...
        // Use the “customer” OBJECT (not customerId)
        'customer'    => $order->customer_id ? ['id' => (string) $order->customer_id] : null,
        // Optional: scheduled ship date etc (ensure ISO string if you include)
        'shipping'    => $order->shipping_scheduled_date ? [
            'scheduledDate' => $this->normalizeIsoDate($order->shipping_scheduled_date),
        ] : null,
    ...
    foreach ($order->items as $i) {
        if (!$i->inventory_id || !$i->quantity) {
            // skip invalid lines – Visma will reject empty inventoryId/quantity
            continue;
        }

        // Either 'inventoryId' OR 'inventory' => ['id' => ...] is OK.
        $line = [
